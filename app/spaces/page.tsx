...
import { SPACES, CITIES, TYPE_LABELS, SpaceType } from "@/lib/spaces";
...
export default function SpacesPage() {
  ...
  }, [search, city, type, maxPrice, onlyAvailable, sort]);
                  ...
                  {CITIES.map((c) => (
                    <button
                      ...
                      className={`text-sm text-left px-3 py-2 rounded-lg transition ${
                        city === c ? "bg-blue-600 text-white" : "text-gray-400 hover:bg-zinc-800"
                      }`}
                    >
                      {c}
                    </button>
              ...
              <div>
                ...
                <div className="flex flex-col gap-1">
                  <button
                    onClick={() => setType("all")}
                    className={`text-sm text-left px-3 py-2 rounded-lg transition ${
                      type === "all" ? "bg-blue-600 text-white" : "text-gray-400 hover:bg-zinc-800"
                    }`}
                  >
                    Todos
                  </button>
                  {TYPES.map(([key, label]) => (
                    <button
                      ...
                      onClick={() => setType(key)}
                      className={`text-sm text-left px-3 py-2 rounded-lg transition ${
                        type === key ? "bg-blue-600 text-white" : "text-gray-400 hover:bg-zinc-800"
                      }`}
                    >
                      {label}
                    </button>
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`w-10 h-5 rounded-full transition-colors relative ${
                    onlyAvailable ? "bg-blue-600" : "bg-zinc-700"
                  }`}
                >
                  <span
                    className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${
                      onlyAvailable ? "left-5" : "left-0.5"
                    }`}
                  />
                </button>
              </div>
